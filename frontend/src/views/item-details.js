...
function ItemDetails () {
  ...
  /*const [inventView, setinventView] = useState([]);
  ...
  useEffect(() =>{
    ...
    const fetchData = async() =>{
      try {
        const response = await fetch(`http://localhost:5000/api/InventoryItems/getAll/${inventView._id}`);
          if (response.ok) {
            const inventData = await response.json(); //parse Json response
            setinventView(inventData); //update state with fetch invent item details

          } else {
            console.error('Error fetching Invent Item:', response.statusText);
          }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    if (inventView && inventView._id) {
      axios.get(`http://localhost:5000/api/InventoryItems/getInventItemDetails/${inventView._id}`)
      .then(response =>{
          ...
          {/*Item Details Display */}
              ...
              <div className="item-details-savebutton">
                <span className="item-details-text48">
                  <span>
                  <button style={{color:'white' , fontWeight:'bold'}}> 
                    <Link to ='/update-invent-item/:id'> EDIT</Link>
                  </button>
                  </span>
                </span>
              </div>
